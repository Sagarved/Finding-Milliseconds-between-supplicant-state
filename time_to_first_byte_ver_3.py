...
file_name = 'M3-Brooklyn.csv'


# Output file
output_file_name = 'M3-Brooklyn'
ap_to_ap_time_list = []
...
def write_to_csv(row_to_write):
    with open('/Users/P2982820/Desktop/' + output_file_name + '_ver_3.csv', 'a') as csvfile:
        # writing row to the output csv
        csvfile.writelines(row_to_write)

# Write in txt file
def write_to_txt(row_to_write):
    with open('/Users/P2982820/Desktop/' + output_file_name + '_ver_3.txt', 'a') as txt_file:
...
with open('/Users/P2982820/Desktop/' + file_name, 'r') as file:
    ...
    for row in reader:
        ...
        # Check if the state is "COMPLETED" after "ASSOCIATING from the same MAC"
            ...
            start_time, end_time, MAC, completed_state = None, None, None, None

            #     end_time_c = datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S.%f')



        # Code for checking previously "Completed state has achieved"
        # Check if the state is "COMPLETED" after "ASSOCIATING from the same MAC"
        elif (state == 'COMPLETED' and start_time is not None and row_MAC == MAC):
            # Setting flag for completed state
            completed_state = True



        # Check if the state is "COMPLETED" after "ASSOCIATING and throughput is greater than zero"
        elif state == '' and tp_dl != '0' and start_time is not None and completed_state:  # and timestamp >= end_time_c:
            ...
            write_to_txt(
                ...
                # Resetting start and end time
            start_time, end_time, MAC, MAC_change, completed_state = None, None, None, None, None
